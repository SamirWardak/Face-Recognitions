...
if authentication_status:
    ...
    while True:
        ...
        face_empty = cv2.resize(face_empty, (360, 400), None, .25, .25)
        faces = face_cascade.detectMultiScale(img, 1.1, 4)
        frame = np.hstack((face_empty, face_empty))
    
    
        if len(faces) == 1:
            x, y, w, h = faces[0]
            cv2.rectangle(img, (x, y), (x + w, y + h), (255, 0, 0), 2)
            name_detected, frame = detecters(img, faces[0], face_empty)
        if len(faces) != 1:
            name_detected = ''
            frame = np.hstack((face_empty, face_empty))
